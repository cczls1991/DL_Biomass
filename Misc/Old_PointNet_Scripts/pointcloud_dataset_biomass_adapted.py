...
class PointCloudsInFiles(InMemoryDataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(self.use_columns) > 0:
            ...
            x = np.empty((self.max_points, 0)) #Create an empty array of same size as max_points if not using any additional columns as features
        coords = coords - np.mean(coords, axis=0)  # centralize coordinates

        #Get plot ID from filename
        plotID = self.files[idx].name.split(".")[0]
        #Load biomass data
        input_table = pd.read_csv(r"/Romeo_Data/Outputs/romeo_plots_mean_height.csv", sep=",", header=0)
        #Extract target value for the correct plot ID
        target = input_table.loc[input_table["PlotID"] == int(plotID)]["zmean"].values

        #Align point cloud with target values
